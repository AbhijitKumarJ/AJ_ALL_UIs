...
class DownloadApp(ctk.CTk):
    ...
    def download_file(self, url):
        try:
            ...
            # Using wget with options to show progress and log all messages
            cmd = ["wget", "-O", output_file, "--progress=bar:force", "--verbose", url]
            
            self.output_queue.put(f"Running command: {' '.join(cmd)}\n")
