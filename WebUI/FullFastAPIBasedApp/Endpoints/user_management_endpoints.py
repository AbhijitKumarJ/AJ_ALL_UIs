...
def get_app_settings():
    ...
    return {"status": "success", "message": "", "data": app_settings}
...
async def create_user_session(
    session: UserSessionCreate, db: orm.Session = Depends(get_db)
):
    folder_name = session_manager.create_session(
        str(session.user_id), session.session_name
    )
    db_session = db_create_user_session(
        db,
        session.user_id,
        session.topic_id,
    ...
    return {
        "status": "success",
        "message": f"Session '{session.session_name}' created",
        "data": db_session,
    }
...
async def get_user_sessions(user_id: int, db: orm.Session = Depends(get_db)):
    ...
    return {
        "status": "success",
        "message": "",
        "data": {"db": db_user_sessions, "logs": session_logs},
    }
